...
### 2026-06-05 — Session: Agent Notes 2 → Phase 0 launch blockers (DONE, 343 tests, pushed e45202e)
- Read `MapleAlerts_AgentNotes_2.docx`; **reconciled against current code** (several notes targeted the retired pre-Aurora `home_screen.dart`). Verified: paywall really was $4.99 (settings $2.99); `AlertType.tax` + tax deadlines genuinely missing; the "duplicate TFSA card" was *dead code*, not a live dupe; vertical deadline list already shipped.
- **P0-1:** paywall → **$2.99/mo · $19.99/yr** (Save 44%), CTA **"Go Pro — $2.99/mo"** (no trial — none configured). Synced price across vision/mvp/monetization/strategy/build-status.
- **P0-2:** added `AlertType.tax` + **free built-in deadlines**: personal tax filing (Apr 30), balance payment (Apr 30), self-employed filing (Jun 15), FHSA contribution (Dec 31), HBP repayment (Mar 1), GST/HST instalments ×4. De-duped the seasonal rail (removed the now-duplicated CRA filing + self-employed entries). `tax`→finance category.
- **P0-3:** removed **11 dead pre-Aurora files** (5 unrouted screens incl. the `_TfsaRoomCard` source + main/tracker/alerts/settings; 6 orphaned widgets + old `utils/theme.dart`) — verified 0 refs, suite green.
- **Decision:** **$2.99/mo · $19.99/yr, no free trial.** Deferred (Agent-Notes Phases 2–3): contextual affiliate CTAs, penalty near-miss celebration, Maple Wrapped, email capture, paywall trigger points, partner/pace/net-worth/calendar/PDF Pro features.
- **Next:** Day 6 release prep (icon, screenshots, signed AAB, Play Data Safety) → Day 7 Play submit + web live. Critical-path: user's Google Play account ($25).
